...
def test_net(net: model.DDPGActor, env: gym.Env, count: int = 10,
    ...
    for _ in range(count):
        obs, _ = env.reset()
        while True:
            obs_v = lib.agent.float32_preprocessor([obs]).to(device)
            mu_v = net(obs_v)
...
if __name__ == "__main__":
    ...
    exp_source = lib.experience.ExperienceSourceFirstLast(env, agent, gamma=GAMMA, steps_count=REWARD_STEPS)
    ...
    frame_idx = 0
    best_reward = None
    with lib.common.utils.RewardTracker(writer) as tracker:
        with lib.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            while True:
                frame_idx += 1
                buffer.populate(1)
