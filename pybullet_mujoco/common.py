...
def unpack_batch_a2c(
    ...
    states_v = lib.agent.float32_preprocessor(states).to(device)
    actions_v = torch.FloatTensor(np.asarray(actions)).to(device)

    # handle rewards
    rewards_np = np.array(rewards, dtype=np.float32)
    if not_done_idx:
        last_states_v = lib.agent.float32_preprocessor(last_states).to(device)
...
@torch.no_grad()
def unpack_batch_sac(batch: tt.List[lib.experience.ExperienceFirstLast],
...
def calc_adv_ref(trajectory: tt.List[lib.experience.Experience],
