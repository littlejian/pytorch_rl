import lib
import lib.ignite as ptan_ignite
...
if __name__ == "__main__":
    ...
    test_env = lib.common.wrappers.wrap_dqn(test_env)
    ...
    agent = lib.agent.PolicyAgent(lambda x: net(x)[0], apply_softmax=True, preprocessor=lib.agent.float32_preprocessor,
                                   device=device)
    if do_distill:
        exp_source = common.DistillExperienceSource(envs, agent, steps_count=1)
    else:
        exp_source = lib.experience.ExperienceSource(envs, agent, steps_count=1)
