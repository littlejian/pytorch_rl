...
import lib
import lib.ignite as lib_ignite
from ignite.engine import Engine
from ignite.metrics import RunningAverage
from ignite.contrib.handlers import tensorboard_logger as tb_logger

def unpack_batch(batch: List[lib.experience.ExperienceFirstLast]):
...
def calc_loss_dqn(
        batch: List[lib.experience.ExperienceFirstLast],
        net: nn.Module,
        tgt_net: nn.Module,
        gamma: float,
...
class EpsilonTracker:
    def __init__(self, selector: lib.actions.EpsilonGreedyActionSelector,
...
def setup_ignite(engine: Engine, params: SimpleNamespace,
    ...
    handler = lib_ignite.EndOfEpisodeHandler(
        exp_source, bound_avg_reward=params.stop_reward)
    handler.attach(engine)
    lib_ignite.EpisodeFPSHandler().attach(engine)

    @engine.on(lib_ignite.EpisodeEvents.EPISODE_COMPLETED)
    ...
    @engine.on(lib_ignite.EpisodeEvents.BOUND_REWARD_REACHED)
    ...
    logdir = f"runs/{params.run_name}-{run_name}"
    ...
    event = lib_ignite.EpisodeEvents.EPISODE_COMPLETED
    tb.attach(engine, log_handler=handler, event_name=event)

    # write to tensorboard every 100 iterations
    lib_ignite.PeriodicEvents().attach(engine)
    metrics = ['avg_loss', 'avg_fps']
    metrics.extend(extra_metrics)
    handler = tb_logger.OutputHandler(
        tag="train", metric_names=metrics,
        output_transform=lambda a: a)
    event = lib_ignite.PeriodEvents.ITERS_100_COMPLETED
    tb.attach(engine, log_handler=handler, event_name=event)
...
class DistillExperienceSource(lib.experience.ExperienceSource):
    """
