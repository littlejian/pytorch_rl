...
def test_model(tiger_net: model.DQNModel, deer_net: model.DQNModel, device: torch.device, args: argparse.Namespace) -> Tuple[float, float, int]:
    env = make_env(args)
    tiger_agent = lib.agent.DQNAgent(
        tiger_net, lib.actions.ArgmaxActionSelector(),
        device)
    deer_agent = lib.agent.DQNAgent(
        deer_net, lib.actions.ArgmaxActionSelector(),
        device)
...
if __name__ == "__main__":
    ...
    tiger_tgt_net = lib.agent.TargetNet(tiger_net)
    print(tiger_net)
    ...
    deer_tgt_net = lib.agent.TargetNet(deer_net)

    action_selector = lib.actions.EpsilonGreedyActionSelector(
        epsilon=PARAMS.epsilon_start)
    epsilon_tracker = common.EpsilonTracker(action_selector, PARAMS)
    tiger_agent = lib.agent.DQNAgent(tiger_net, action_selector, device)
    deer_agent = lib.agent.DQNAgent(deer_net, action_selector, device)
    ...
    def process_batch(engine, batch):
        ...
        if tiger_batch:
            ...
            tiger_loss_v = model.calc_loss_dqn(
                ...
                lib.agent.default_states_preprocessor,
    ...
    @engine.on(lib_ignite.PeriodEvents.ITERS_10000_COMPLETED)
