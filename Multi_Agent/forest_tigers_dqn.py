...
def test_model(net: model.DQNModel, device: torch.device, args: argparse.Namespace) -> Tuple[float, float]:
    ...
    tiger_agent = lib.agent.DQNAgent(
        net, lib.actions.ArgmaxActionSelector(),
        device)
...
if __name__ == "__main__":
    ...
    action_selector = lib.actions.EpsilonGreedyActionSelector(epsilon=PARAMS.epsilon_start)
    epsilon_tracker = common.EpsilonTracker(action_selector, PARAMS)
    tiger_agent = lib.agent.DQNAgent(net, action_selector, device)
    deer_agent = data.RandomMAgent(env, env.handles[0])
    ...
    buffer = lib.experience.ExperienceReplayBuffer(exp_source, PARAMS.replay_size)
